...
void main() {
  ...
  float glowMask = pow(abs(n2), 2.5) * 0.5;

  vec3 col;
  if (u_light > 0.5) {
    // Modo claro: el color se acerca tiñendo el fondo (efecto tinta/acuarela),
    // en vez de brillar aditivamente sobre negro.
    col = u_bg;
    col = mix(col, u_colors[0], smoothstep(0.1, 1.0, n1) * 0.5);
    col = mix(col, u_colors[1], clamp(glowMask * 1.6, 0.0, 1.0));
  } else {
    // Modo oscuro: comportamiento original, brillo aditivo sobre el fondo.
    col = u_bg;
    col += u_colors[0] * smoothstep(0.1, 1.0, n1) * 0.5;
    col += u_colors[1] * glowMask;
  }
  ...
  col = mix(u_bg, col, smoothstep(1.2, 0.2, dist));
...
export interface AuralisProps {
  colors?: string[];
  bg?: string;
  /** "dark": brillo aditivo sobre el fondo (glow). "light": mezcla/tiñe el fondo (tinta). */
  variant?: "dark" | "light";
  speed?: number;
  grain?: number;
  height?: string;
...
const Auralis = ({
  ...
  useEffect(() => {
    ...
    const render = (t: number) => {
      ...
      gl.uniform1f(locs.light, variant === "light" ? 1 : 0);
